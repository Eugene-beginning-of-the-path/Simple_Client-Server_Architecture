...
int main()
{
	...
	hints.ai_flags = AI_PASSIVE; //флаг добавляется только для пассивной стороны (клиент - активная сторона, ищет с кем ему законнектиться), сокет ждет соединения
	...
	lisetnSocket = socket(addrResult->ai_family, addrResult->ai_socktype, addrResult->ai_protocol); //создаем сокет на базе найденного Сервер-сокета addrResult
	if (lisetnSocket == INVALID_SOCKET)
	{
	...
	errorStat = bind(lisetnSocket, addrResult->ai_addr, (int)addrResult->ai_addrlen); //привязываем прослушивающий сокет к определенному адресу 
	if (errorStat == SOCKET_ERROR)
	{
		cout << "Binding socket failed";
		closesocket(lisetnSocket);
		lisetnSocket = INVALID_SOCKET;

		freeaddrinfo(addrResult);
		WSACleanup();
		return 1;
	}
	cout << "Server is waiting a signal from client socket..." << endl;
	...
	do
	{
		...
		if (errorStat > 0)
		{
			cout << "Received a message from Client (2): " << endl;
			cout << "Received " << errorStat << " bytes" << endl;
			cout << "Received data: " << recvBuffer << endl;
			cout << "Personse sent to Client with message - " << sendBuffer << " (3)" << endl;
			errorStat = send(toClientSocket, sendBuffer, (int)strlen(sendBuffer), 0);
			if (errorStat == SOCKET_ERROR)
			{
